...
```matlab
% 
function [position, value] = HCLPSO(num_particle, range, dimension, max_iteration, max_FES, fobj) 

disp('HCLPSO 正在为您的问题估计全局最优解....')

check_vel = [];
check_vel1 = [];
check_vel2 = [];
convergencecurve = zeros(1, max_iteration);
con_curve = zeros(1, max_FES);

num_g = num_particle;
num_g1 = 2;
num_g2 = num_g - num_g1;

j = 0:(1 / (num_g - 1)):1; % 学习概率曲线 Pc
j = j * 10;
Pc = ones(dimension, 1) * (0.0 + ((0.25) * (exp(j) - exp(j(1))) / (exp(j(num_g)) - exp(j(1))))); 

Weight = 0.99 - (1:max_iteration) * 0.79 / max_iteration; % 惯性权重
K = 3 - (1:max_iteration) * 1.5 / max_iteration; % 加速系数
c1 = 2.5 - (1:max_iteration) * 2 / max_iteration;
c2 = 0.5 + (1:max_iteration) * 2 / max_iteration;

% 初始化
range_min = repmat(range(1, :), num_g, 1); % 初始群体元素的范围
range_max = repmat(range(2, :), num_g, 1);
interval = range_max - range_min;
v_max = interval * 0.2; 
v_min = -v_max;
pos = range_min + interval .* rand(num_g, dimension);  % 位置
vel = v_min + (v_max - v_min) .* rand(num_g, dimension); % 速度
...
for i = num_g1 + 1:num_g % 更新群体 2 的示例
    fri_best(i, :) = i * ones(1, dimension);
...
while k <= max_iteration && fitcount <= max_FES
       ...
       % 群体 2
        ...
        % 评估适应度
         for i = 1:num_g   
            if (sum(pos(i, :) > range_max(i, :)) + sum(pos(i, :) < range_min(i,
 ...
 :)) == 0)
            ...
            if  result(i) < pbest_val(i) % 更新 pbest 值和位置
                pbest_pos(i, :) = pos(i, :);   
            ...
            if  pbest_val(i) < gbest_val % 更新 gbest 值和位置
        ...
        for i = 1:num_g1 % 更新群体 1 的示例
            if obj_func_slope(i) > 5
                fri_best(i, :) = i * ones(1, dimension); % 对自己的 pbest
                friend1 = ceil(num_g1 * rand(1, dimension));
                friend2 = ceil(num_g1 * rand(1, dimension));
                friend = (pbest_val(friend1) < pbest_val(friend2)) .* friend1 + (pbest_val(friend1) >= pbest_val(friend2)) .* friend2;
                toss = ceil(rand(1, dimension) - Pc(:, i)');
    ...
    convergencecurve(k) = gbest_val;
    con_curve(fitcount) = gbest_val;
