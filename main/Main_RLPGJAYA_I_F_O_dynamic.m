...
for j=1:Number_of_runs
    tic;
   [Best_pos(j,:),Best_score(j)]=RLPGJAYA1(N , range ,dim,Max_iter,NEF,fobj);   
   RLPGJAYA_time(j)=toc;
  end
save('../data/RLPGJAYA_FO_dynamic') % save results of the fractional order model

%save('../data/RLPGJAYA_IO_dynamic') % save results of the integer order model
% % %
...
load('../data/RLPGJAYA_FO_dynamic.mat')
[Mm  II]=min(Best_score);
Mm_std=std(Best_score);
x=Best_pos(II,:);
...
i_load_integer=-A_L*exp(-tim/T_l)+A_C*exp(-tim/T_l)+i_inf;
RLPGJAYA_FO_rms=rms (I_Load_inter-Y2');
